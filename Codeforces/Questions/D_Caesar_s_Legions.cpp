...
// 12*3*100*100
ll dp[101][101][11][2];
ll f(ll n1, ll n2, ll last, ll type, ll k1, ll k2)
{
    if (type == 0 && last > k1)
    {
        return 0;
    ...
    if (dp[n1][n2][last][type] != -1)
        return dp[n1][n2][last][type];
    ll ans = 0;
    if (n1 > 0)
    {
        if (type == 0)
        {
            ans = (ans + f(n1 - 1, n2, last + 1, 0, k1, k2)) % MOD;
        }
        else
        {
            ans = (ans + f(n1 - 1, n2, 1, 0, k1, k2)) % MOD;
        }
    }
    if (n2 > 0)
    {
        if (type == 0)
        {
            ans = (ans + f(n1, n2 - 1, 1, 1, k1, k2)) % MOD;
        }
        else
        {
            ans = (ans + f(n1, n2 - 1, last + 1, 1, k1, k2)) % MOD;
        }
    }
    return dp[n1][n2][last][type] = ans;
}
