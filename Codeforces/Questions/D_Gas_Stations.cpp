...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (suf[i] == -1)
        {
            ll can = min(c - curr, b / nums[i]);
            curr += can;
            b -= can * nums[i];
        }
        else if (arr[suf[i]] > curr + arr[i])
        {
            ll req = arr[suf[i]] - arr[i] - curr;
            ll can = min({c, req, b / nums[i]});
            curr += can;
            b -= can * nums[i];
        }
        // else if (curr + arr[i] >= arr[suf[i]])
        // {
        //     continue;
        // }
