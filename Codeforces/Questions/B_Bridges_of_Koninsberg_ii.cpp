...
void solve() {
    ...
    vector<vector<long long>> nums(n + 2, vector<long long>(n + 2, 0));
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            long long up = (i - 1 >= 0) ? nums[i - 1][j] : 0;
            long long left = (j - 1 >= 0) ? nums[i][j - 1] : 0;
            long long top = (i - 1 >= 0 && j - 1 >= 0) ? nums[i - 1][j - 1] : 0;
            nums[i][j] += up + left - top;
        }
    }
