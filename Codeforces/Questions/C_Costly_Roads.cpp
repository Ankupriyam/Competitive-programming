...
// ll times=0;
pair<ll, ll> f(ll i, ll parent,
               vector<vector<vector<ll>>> &adj,
               unordered_map<unsigned long long, pair<ll, ll>> &dp)
{
    unsigned long long key =
        (static_cast<unsigned long long>(parent + 1) << 32) |
    ...
    for (auto &e : adj[i])
    {
        ...
        if (child != parent)
        {
            auto z = f(child, i, adj, dp);

            ans += weight * z.second + z.first;
            count += z.second;
        }
    }

    return dp[key] = {ans, count};
}
