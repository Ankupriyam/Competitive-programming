...
ll f(vector<ll> &nums, int n)
{

    vector<ll> x1(n / 2), x2(n / 2), pmax(n / 2), pmin(n / 2, 0), smax(n / 2), smin(n / 2);
    for (int i = 0; i < n / 2; i++)
    {
        x1[n / 2 - i - 1] = nums[i] + nums[n - i - 1];
    }
    for (int i = 0; i < n / 2; i++)
    {
        x2[n / 2 - i - 1] = nums[i] + nums[n - i - 2];
    }
    ...
    for (int i = 0; i < n / 2; i++)
    {
        ...
        if (i != n / 2 - 1)
        {
            maxi = max(maxi, smax[i + 1]);
            mini = min(mini, smin[i + 1]);
        }
