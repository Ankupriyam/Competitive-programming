...
void solve()
{
    ...
    cin >> n;
    vector<ll> nums(n);
    vector<vector<ll>> pref(32, vector<ll>(n, 0));
    for (int i = 0; i < n; i++)
    {
        cin >> nums[i];
    }
    ...
    while (q--)
    {
        ...
        for (; i >= 0; i--)
        {
            if (((k >> i) & 1) == 1 && ((nums[l] >> i) & 1) == 0)
            {
                ans2 = -1;
                break;
            }
            else if (((k >> i) & 1) == 1 && ((nums[l] >> i) & 1) == 1)
            {
                ll low = l, high = n - 1, curr = -1;
                while (low <= high)
                {
                    ll mid = low + (high - low) / 2;
                    if (pref[i][mid] - mid == pref[i][l] - l)
            ...
            else if (((nums[l] >> i) & 1) == 1)
            {
                ll low = l, high = n - 1, curr = l;
                while (low <= high)
                {
                    ll mid = low + (high - low) / 2;
                    if (pref[i][mid] - mid == pref[i][l] - l)
