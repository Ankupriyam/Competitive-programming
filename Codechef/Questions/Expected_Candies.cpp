...
double f(int l, int r, vector<ll>& nums)
{
    if (l > r)
        return 0;
...
void solve()
{
    ...
    fill(&dp[0][0], &dp[0][0] + 1005 * 1005, -1e18);

    double diff = f(0, n - 1, nums);
    ...
    cout << fixed << setprecision(15)<< (total + diff) / 2.0;
}  
