...
// ll f(int i, int j, vector<vector<ll>> &vis, int x, int y, int n, vector<vector<ll>> &valid)
// {
// }
...
void solve()
{
    ...
    cin >> n;
    // vector<ll> nums(n);
    vector<vector<ll>> nums(n + 1, vector<ll>(2 * (n + 1) - 1, 0)), valid(n + 1, vector<ll>(2 * (n + 1) - 1, 0)), vis(n + 1, vector<ll>(2 * (n + 1) - 1, 0));
    for (int i = 1; i <= n; i++)
    {
    ...
    while (!q.empty())
    {
        ...
        while (len--)
        {
            ...
            if (j % 2 == 0)
            {
                if (i + 1 <= n && valid[i + 1][j + 1] == 1 && !vis[i + 1][j + 1])
                {
                    vis[i + 1][j + 1] = 1;
                    // f(i + 1, j + 1, vis, x, y, n, valid);
                    q.push({i + 1, j + 1});
                }
            }
            else
            {
                if (i - 1 > 0 && j > 0 && valid[i - 1][j - 1] == 1 && !vis[i - 1][j - 1])
                {
                    vis[i - 1][j - 1] = 1;
                    // f(i + 1, j + 1, vis, x, y, n, valid);
                    q.push({i - 1, j - 1});
            ...
            if (j > 0 && !vis[i][j - 1] && valid[i][j - 1])
            {
                vis[i][j - 1] = 1;
                // f(i, j - 1, vis, x, y, n, valid);
                q.push({i, j - 1});
            }
            if (j + 1 <= 2 * i - 2 && !vis[i][j + 1] && valid[i][j + 1])
            {
                vis[i][j + 1] = 1;
                // f(i, j + 1, vis, x, y, n, valid);
                q.push({i, j + 1});
